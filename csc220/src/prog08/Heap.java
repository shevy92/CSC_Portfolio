...
public class Heap <E extends Comparable<E>> {
  ...
  public E poll() {
    ...
    if (theData.get(0) == null) {
        return null;
    }
    // Save the top of the heap.
    E topOfHeap = theData.get(0);
    ...
    while (    (left < theData.size() && compare(left, parent) < 0)
    		|| (right < theData.size() && compare(right, parent) < 0)) {
    	
      int swap = left;
      if (right < theData.size() || compare(left, right) > 0) {
    	  swap = left;
      } else {
    	  swap = right;
      }	
